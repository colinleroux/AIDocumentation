# Home / Overview

Welcome. This site is where I keep my working AI engineering playbook: what I build, what fails, and what actually works in production.

## What This Site Is

A practical knowledge base for designing, building, and improving AI systems. It is intentionally implementation-first.

## My Current Stack

- Python
- Flask
- Docker
- API-hosted LLMs
- Local models with Ollama
- RAG pipelines

## What I'm Learning (Now)

- Retrieval quality and RAG evaluation
- Better tool-calling reliability
- Faster local model workflows
- Simpler, testable AI architecture

## Key Projects

- [MVP Internal RAG](projects/mvp-internal-rag.md)
- [docx2html Pipeline](projects/docx2html-pipeline.md)
- [Internal RAG System](projects/internal-rag-system.md)
- Prompt and tool orchestration prototypes
- Model and chunking experiments

## Learning Philosophy

Build small, measure early, keep systems observable, and prefer repeatable patterns over one-off cleverness.

## Quick Navigation

- [AI Engineering Fundamentals](ai-fundamentals.md)
- [RAG Systems](rag/index.md)
- [Local Models](local-models/index.md)
- [Projects](projects/index.md)
- [Patterns & Recipes](patterns/index.md)
- [Tooling & Stack](tooling.md)
- [Formal Learning Approach](formal-learning-approach.md)
- [Experiments & Findings](experiments/index.md)
- [Mistakes & Lessons Learned](lessons.md)
- [Future Ideas / Roadmap](roadmap.md)
- [Architecture Diagrams](architecture.md)
- [Glossary](glossary.md)
